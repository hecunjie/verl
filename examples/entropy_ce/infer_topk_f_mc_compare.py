        ...
        if should_branch:
            if policy == "min_f_mc":
                ...
                if remaining <= 0:
                    ...
                    if selection_f_mode in ("greedy_path", "mc", "mc_max", "mc_mid"):
                        ...
                        if not f_values or len(f_values) != len(cands):
                            ...
                            elif selection_f_mode == "mc_mid":
                                order = np.argsort(arr_f)
                                best_i = int(order[len(order) // 2])
                            else:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "mc_mid (median expected F by multi-sample), "
            "random_topk (uniform random among top-k candidates), "
